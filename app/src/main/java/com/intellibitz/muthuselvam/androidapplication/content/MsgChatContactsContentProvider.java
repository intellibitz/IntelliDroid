...
import static com.intellibitz.muthuselvam.androidapplication.content.MsgChatContactContentProvider.TABLE_MSGCHATCONTACT;
...
public class MsgChatContactsContentProvider extends
    ...
    public static String AUTHORITY = "com.intellibitz.muthuselvam.androidapplication.content.MsgChatContactsContentProvider";
...
//                        " ak.intellibitz_id as akonid, ak.status as akstatus, " +
                        ...
                        "] " +
                        "left outer join " + TABLE_MSGCHATCONTACT +
...
//        Cursor cursor = getAKContactCursorJoin(db, item, id);
        Cursor cursor = databaseHelper.query(db, TABLE_MSGCHATCONTACT,
//                projection
...
//        message thread is the parent, email item child
            ...
            _id = databaseHelper.insert(db, TABLE_MSGCHATCONTACT, null, values);
            item.set_id(_id);
        } else {
...
//        message thread is the parent, email item child
        MsgChatContactContentProvider.fillContentValuesFromContactItem(item, values);
        databaseHelper.update(db, TABLE_MSGCHATCONTACT, values,
                ContactItemColumns.KEY_ID + " = ?",
...
//                selection args
        ...
        if (null == cursor || 0 == cursor.getCount()) {
            ...
            item.set_id(_id);
//            updateMessageThreadEmail(item, id);
            databaseHelper.update(db, TABLE_MSGCHATCONTACT, values, ContactItemColumns.KEY_ID + " = ?",
                    new String[]{String.valueOf(_id)});
        }
//        creates the join
        createOrUpdateContactsContactJoin(databaseHelper, db, _id, id);
...
//            deletes the item
            _id = databaseHelper.delete(db, TABLE_MSGCHATCONTACT,
                    ContactItemColumns.KEY_ID + " = ? ",
...
//        deletes the join
        ...
        if (0 == row) return 0;
//            deletes the item
        return databaseHelper.delete(db, TABLE_MSGCHATCONTACT,
                ContactItemColumns.KEY_ID + " = ? ",
                new String[]{String.valueOf(_id)});
    }
